...
function Stats() {
  const listOfTasks = useSelector((state: TaskRelatedState) => state.tasksRelated.filtredLengths);
  const [currentQuantityUrgent, setCurrentQuantityUrgent] = useState<number | string>(2);
  function handleInput(event:ChangeEvent<HTMLInputElement>): void {
    setCurrentQuantityUrgent(event.currentTarget.value);
  }
  return (
    <section>
      ...
      <ul className="text-right italic md:text-lg text-sm">
        {listOfTasks.completed > 0 && <li>You have finished {listOfTasks.completed} task{listOfTasks.completed === 1 ? '':'s'} succefully. </li>}
